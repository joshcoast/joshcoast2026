...
if ( class_exists( 'GenerateBlocks_Libraries' ) ) :
	...
	class GenerateBlocks_Pro_Pattern_Library extends GenerateBlocks_Pro_Singleton {
		...
		public function init() {
			...
			add_action( 'save_post_wp_block', [ $this, 'after_save' ], 10, 3 );
		}
		...
		protected function get_default(): GenerateBlocks_Library_DTO {
			return ( new GenerateBlocks_Library_DTO() )
				...
				->set( 'public_key', 'c4ngBQvKWeqG17W8SAWVJ0FuyD9uCVvq' )
				->set( 'is_enabled', true )
				->set( 'is_default', true );
		}
		...
		public function build_tree( string $post_content, $post_id ): array {
			...
			if ( strpos( $preview, 'gb-accordion' ) !== false && isset( $wp_scripts->registered['generateblocks-accordion'] ) ) {
				$script = $wp_scripts->registered['generateblocks-accordion'];
				$scripts[] = $script->src;
			}

			if ( strpos( $preview, 'gb-tabs' ) !== false && isset( $wp_scripts->registered['generateblocks-tabs'] ) ) {
				$script = $wp_scripts->registered['generateblocks-tabs'];
				$scripts[] = $script->src;
			}
